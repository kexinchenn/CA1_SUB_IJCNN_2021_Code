...
public:
	...
	void run(const ParameterInstances &parameters, std::ostream &outputStream) const {
		...
		getRealTestFRs(numNeurons, numNeuronsAll, numPaths, trueTeFR);
		...
		// float inpAVToInhMaxWght, inpLVToInhMaxWght, inpHDToInhMaxWght, inpPosToInhMaxWght;
		...
		for (int i = 0; i < indiNum; i++){
			...
			// inpAVToInhMaxWght = parameters.getParameter(i,14);
			...
			// 	RangeWeight(0.0f,inpAVToExcMaxWght*initWtScale,inpAVToExcMaxWght), 0.5f, RangeDelay(1), RadiusRF(-1), SYN_PLASTIC);
			// inLinVelToExc[i] = network->connect(gInLinVel, exc[i], "random", 
			// 	RangeWeight(0.0f,inpLVToExcMaxWght*initWtScale,inpLVToExcMaxWght), 0.5f, RangeDelay(1), RadiusRF(-1), SYN_PLASTIC);
			// inHDToExc[i] = network->connect(gInHD, exc[i], "random", 
			// 	RangeWeight(0.0f,inpHDToExcMaxWght*initWtScale,inpHDToExcMaxWght), 0.5f, RangeDelay(1), RadiusRF(-1), SYN_PLASTIC);
			// inPosToExc[i] = network->connect(gInPos, exc[i], "random", 
			// 	RangeWeight(0.0f,inpPosToExcMaxWght*initWtScale,inpPosToExcMaxWght), 0.05f, RangeDelay(1), RadiusRF(-1), SYN_PLASTIC);

			// // connections from input groups to inh group
			// inAngVelToInh[i] = network->connect(gInAngVel, inh[i], "random", 
			// 	RangeWeight(0.0f,inpAVToInhMaxWght*initWtScale,inpAVToInhMaxWght), 0.5f, RangeDelay(1), RadiusRF(-1), SYN_PLASTIC);
			// inLinVelToInh[i] = network->connect(gInLinVel, inh[i], "random", 
			// 	RangeWeight(0.0f,inpLVToInhMaxWght*initWtScale,inpLVToInhMaxWght), 0.5f, RangeDelay(1), RadiusRF(-1), SYN_PLASTIC);
			// inHDToInh[i] = network->connect(gInHD, inh[i], "random", 
			// 	RangeWeight(0.0f,inpHDToInhMaxWght*initWtScale,inpHDToInhMaxWght), 0.5f, RangeDelay(1), RadiusRF(-1), SYN_PLASTIC);
			// inPosToInh[i] = network->connect(gInPos, inh[i], "random", 
			// 	RangeWeight(0.0f,inpPosToInhMaxWght*initWtScale,inpPosToInhMaxWght), 0.05f, RangeDelay(1), RadiusRF(-1), SYN_PLASTIC);
		...
		for (int total = 0; total < totalTrTrials; total++) {
			...
			for (int reps = 0; reps < numPaths; reps++) {
				...
				for (int z = 0; z < repTrialsTe; z++) {
					...
					coordsFR = gauss2d(posXY[0], posXY[1], xCoordPref, yCoordPref, INP_POS_X, INP_POS_Y, numBins, sigma_x, r_max);						
